<x-panitia-layout title="Data Formulir">
    <section class="space-y-6">
        <div class="rounded-[2rem] bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.22em] text-slate-500">Formulir</p>
                    <h2 class="mt-2 text-2xl font-bold text-slate-900">Data Formulir</h2>
                    <p class="mt-2 text-sm text-slate-500">Pantau formulir yang sudah tersimpan dari akun orang tua, lihat status pengisiannya, lalu ekspor data bila diperlukan.</p>
                </div>
        ...
        <div class="grid gap-5 xl:grid-cols-2">
            <article class="rounded-[1.75rem] bg-white p-6 shadow-sm ring-1 ring-slate-200">
                ...
                <p class="mt-3 text-sm text-slate-500">Tercatat dari seluruh formulir yang sudah dibuat melalui akun orang tua.</p>
            </article>
            ...
            <article class="rounded-[1.75rem] bg-white p-6 shadow-sm ring-1 ring-slate-200">
                ...
                <div class="mt-4 flex flex-wrap items-center gap-3 text-sm text-slate-500">
                    <span class="inline-flex rounded-full bg-blue-100 px-3 py-1 font-semibold text-blue-700">{{ number_format($stats['completion_rate'], 1) }}%</span>
            ...
            <div class="border-b border-slate-200 p-6">
                <form method="GET" class="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_220px_220px_auto]">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a
                            href="{{ route('panitia.forms.export', ['q' => $search, 'status' => $status, 'ta' => $academicYear]) }}"
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-[#f5f8fc] text-slate-500">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left font-bold uppercase tracking-[0.08em]">Tanggal Beli</th>
                            <th class="px-6 py-4 text-left font-bold uppercase tracking-[0.08em]">Harga</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @forelse ($forms as $form)
                            <tr class="align-top">
                                <td class="px-6 py-5 font-semibold text-slate-900">{{ $form->display_form_number }}</td>
                                <td class="px-6 py-5">
                                    <p class="font-semibold text-slate-900">{{ $form->display_buyer_name }}</p>
                                    <p class="mt-1 text-xs text-slate-500">{{ $form->user?->email ?? '-' }}</p>
                                </td>
                                <td class="px-6 py-5 text-slate-700">{{ $form->display_student_name }}</td>
                                <td class="px-6 py-5 text-slate-700">{{ $form->display_purchase_date }}</td>
                                <td class="px-6 py-5 text-slate-700">{{ $form->display_price }}</td>
                                <td class="px-6 py-5">
                                    @php
                                        $badgeClasses = match ($form->display_status_tone) {
                                    ...
                                    <span class="inline-flex rounded-full px-3 py-1 text-sm font-semibold {{ $badgeClasses }}">
                                        {{ $form->display_status_label }}
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-slate-500">Belum ada data formulir yang bisa ditampilkan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
